...
contract Name is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100_000_000 * 10**18; //100 Million Total Supply; Underscore used for readability
    ...
    uint256 public TotalFee = _taxFee.add(_liquidityFee).add(_burnFee).add(_charityFee);

    uint256 public divisor = 1000; // For better percision in tax calculations
    ...
    uint256 private numTokensSellToAddToLiquidity = (_tTotal/500); //0.2% of the Total Supply
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 burnAmt = amount.mul(_burnFee).div(divisor);
        uint256 charityAmt = amount.mul(_charityFee).div(divisor);
    ...
    function UpdateFees(uint256 taxFee, uint256 liquidityFee, uint256 charityFee, uint256 burnFee ) external onlyOwner() {
        require( taxFee.add(liquidityFee).add(burnFee).add(charityFee) <= 200, "You can not set more than 20% tax.");
        _taxFee = taxFee;
        _liquidityFee = liquidityFee;
        _charityFee = charityFee;
        _burnFee = burnFee;

        TotalFee = _taxFee.add(_liquidityFee).add(_burnFee).add(_charityFee);
    }
    ...
    function UpdatenumTokensSellToAddToLiquidity(uint256 amount) public onlyOwner{
        require(amount >= (_tTotal/1000),"You can not set amount less than 0.1% of the total supply.");
